...
std::tuple<std::vector<i_mat>, d_mat> ParseMnistImages(std::string);
i_vec ParseMnistLabels(std::string);

int main(){

    //images
    auto TrainImagesTuple = ParseMnistImages("train-images.idx3-ubyte");
    auto PrintingTrainImages = std::get<0>(TrainImagesTuple);
    auto FlatTrainImages = std::get<1>(TrainImagesTuple);

    //labels
    auto bar = ParseMnistLabels("train-labels.idx1-ubyte");
